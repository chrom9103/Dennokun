...
export default function MatchResultExport({
  ...
  const mainRoundSegments = segments
    .filter((s) => !s.is_pre_round)
    .sort((a, b) => (a.order_number ?? 0) - (b.order_number ?? 0));

  function buildFlatRows(segs: typeof segments): FlatRowItem[] {
  ...
  const preRows = buildFlatRows(preRoundSegments);
  const mainRows = buildFlatRows(mainRoundSegments);

  function renderTable(flatRows: FlatRowItem[], headingId: string) {
  ...
  return (
    ...
    >
      ...
      {renderTable(preRows, "match-result-pre-round")}
      ...
      {renderTable(mainRows, "match-result-main-round")}
    </div>
  );
}
